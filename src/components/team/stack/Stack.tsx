...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElement = useWindowScroll
      ? (document.querySelector(`.${styles.scrollStackEnd}`) as HTMLElement | null)
      : (scrollerRef.current?.querySelector(`.${styles.scrollStackEnd}`) as HTMLElement | null);
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      const lenis = new Lenis({
        ...
        touchMultiplier: 2,
        infinite: false,
        // @ts-expect-error (some Lenis options may not be in types)
  ...
  useLayoutEffect(() => {
    ...
    const cards = Array.from(
      useWindowScroll
        ? (document.querySelectorAll(`.${styles.scrollStackCard}`) as NodeListOf<HTMLElement>)
        : (scroller.querySelectorAll(`.${styles.scrollStackCard}`) as NodeListOf<HTMLElement>)
    );
